...
public class Conventions {
  ...
  public static void initProfile() {
    try {
      ...
      props.load((org.beanfuse.commons.web.dispatch.Conventions.class).getResourceAsStream("/convention_dispatch.properties"));
      ...
      do {
        ...
        if (StringUtils.isEmpty(profile.ctlPostfix))
        ...
        if (!profile.pagePrefix.endsWith(separator + ""))
          profile.pagePrefix += separator + "";
        ...
        if (null != simpleURIStyle && "false".equals(simpleURIStyle))
          profile.simpleURIStyle = Boolean.FALSE;
        else
          profile.simpleURIStyle = Boolean.TRUE;
  ...
  public static StringBuffer getViewName(HttpServletRequest request, Class controllerClass, String relativePath) {
    if (StringUtils.isNotEmpty(relativePath) && relativePath.startsWith(separator + ""))
      return new StringBuffer(relativePath);
    Profile profile = getProfile(controllerClass);
    if (null == profile)
      throw new RuntimeException("no convention profile for " + controllerClass);
    ...
    if (StringUtils.isEmpty(relativePath)) {
      ...
      if (null == commonMapping.get(method))
        buf.append(method);
      else
        buf.append(commonMapping.get(method));
    } else {
  ...
  static {
    logger = LoggerFactory.getLogger(org.beanfuse.commons.web.dispatch.Conventions.class);
    initProfile();
